// Servidor local Hauscrete Baja — Ruta A (SQLite en archivo, sin dependencias npm).
// Sirve el HTML del CRM y persiste window.storage en baja.sqlite.
//
// Uso:  npm start   ->  http://localhost:3200
//
...
// Baja corre LOCAL (sin secretos de Fly): si no viene el token del bot por variable
// de entorno, se toma del archivo telegram-token.txt junto al código (una sola línea).
// El archivo está excluido de git y de la imagen de Docker.
try {
  if (!process.env.TELEGRAM_BOT_TOKEN) {
    const t = fs.readFileSync(path.join(__dirname, 'telegram-token.txt'), 'utf8').trim();
    if (t) process.env.TELEGRAM_BOT_TOKEN = t;
  }
} catch { /* sin archivo: la integración de Telegram queda inactiva */ }
...
const DB_FILE = process.env.DB_FILE || path.join(__dirname, 'baja.sqlite');
// Contraseña de acceso opcional (Basic Auth). Si se define ACCESO_PASS, el sitio la pide.
const ACCESO_USER = process.env.ACCESO_USER || 'hausbaja';
...
async function enviarReporteAdmin(state, TG, REP) {
  const texto = REP.repAdminTexto(state, 'Hauscrete Baja');
...
async function enviarReporteDepositos(state, TG, REP) {
  const texto = REP.repDepositosTexto(state, 'Hauscrete Baja');   // todos los depósitos, con quién los fijó
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (ACCESO_PASS) {
      ...
      if (!ok) { res.writeHead(401, { 'WWW-Authenticate': 'Basic realm="Hauscrete Baja"' }); res.end('Acceso restringido'); return; }
    }
    ...
    /* ---- e.firma para bajar facturas del SAT ----
       ...
       en la computadora, de la carpeta SAT-Baja.
    ...
    if (p.startsWith('/api/sat/')) {
      ...
      const DIR_SAT = path.join(os.homedir(), 'SAT-Baja');
      ...
      if (p === '/api/sat/guardar' && req.method === 'POST') {
        ...
        const cfg = {
          ...
          crm: { url: b.crmUrl || 'https://hauscrete-baja-erp.fly.dev', usuario: b.crmUsuario || 'hausbaja', password: b.crmPassword || '' },
        };
    ...
    if (p === '/api/reporte-depositos/previa' && req.method === 'GET') {
      const REP = await import('./reporte-admin.mjs');
      const state = JSON.parse(leerStateValue() || '{}');
      const texto = REP.repDepositosTexto(state, 'Hauscrete Baja') || '(hoy no hay depósitos programados)';
      const destinatarios = (state.asesores || []).filter((a) => a.tgInfoAdmin && a.tg && a.tg.chatId)
        .map((a) => ((a.nombre || '') + ' ' + (a.apellidos || '')).trim());
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`\n  Hauscrete Baja ERP  ·  puerto ${PORT}`);
